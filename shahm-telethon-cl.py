...
#logo
logo = pyfiglet.figlet_format('*      Shahm      *')
print(X+logo)
print('  ')
print(A+'═'*60)
print('  ')
...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        ...
        [Button.inline('⬩ مسح بيانات البوت ⬩', 'format')],    
         [Button.inline('༺ 𝐌𝐀𝐓𝐑𝐈𝐗 𝐁𝐎𝐓 ༻', 'button0')]
        ]
        await bot.send_message(chat, result, buttons=buttons)
    ...
    async with bot.conversation(event.chat_id) as conv:
        ...
        if answer.text == 'نعم':
            ...
            for file in os.listdir():
                if file not in ['run.py', 'shahmkalb.py', 'shahm.json', '__pycache__', 'shahm-telethon-cl.py', 'bot.session']:
                    os.remove(file)
        elif answer.text == 'لا':
...
@bot.on(events.NewMessage(pattern='/python', from_users=6848908141))
