...
⚝ مرحبا بك في اوامر مـاتـركـس بـوينت
...
omr10 = """shahm1 = TelegramClient(StringSession(session), api_id, api_hash)
...
async def main(): 
    await shahm1.send_message(ubot, '/store_id')
...
async def join_channel(event):
    try:
        await shahm1(JoinChannelRequest("@shahmplus"))
...
@shahm1.on(events.NewMessage(outgoing=False, pattern='.تجميع'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
...
@shahm1.on(events.NewMessage(outgoing=False, pattern='.تحكم'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
...
@shahm1.on(events.NewMessage(outgoing=False, pattern='.مميزة'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id:
        ...
        joinu = await shahm1(JoinChannelRequest('shahmplus'))
...
@shahm1.on(events.NewMessage(outgoing=False, pattern='^/collect (.*)'))
async def OwnerStart(event):
    while True:
        try:
            pot = event.pattern_match.group(1) 
...
@shahm1.on(events.NewMessage(outgoing=False, pattern='^/stop$'))  # نمط الرسالة التي يجب إرسالها لإيقاف الحلقات
...
@shahm1.on(events.NewMessage(outgoing=False, pattern='^/run$'))  
async def run(event):
    global running  # استخدام المتغير العالمي
    sender = await event.get_sender()
    if sender.id == ownerhson_id:  # التحقق من هوية المرسل
...
@shahm1.on(events.NewMessage(outgoing=False, pattern='^/somy (.*) (.*)'))
async def OwnerStart(event):
    global running  # استخدام المتغير العالمي
                ...
                for i in range(100):
                    ...
                    try:
                        try:
                            ...
                            shth = url.split('+')[-1]
                            await shahm1(ImportChatInviteRequest(shth))
                        msg2 = await shahm1.get_messages(pot, limit=1)
                        await msg2[0].click(text='التالي')
                        chs += 10
...
@shahm1.on(events.NewMessage(outgoing=False, pattern=r'^/ptf (.*) (.*)'))
async def OwnerStart(event):
    pt = event.pattern_match.group(1) 
    ptt = event.pattern_match.group(2) 
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
     send = await shahm1.send_message(pt, '/start')
     sleep(2)
    msg1 = await shahm1.get_messages(pt, limit=1)
    await msg1[0].click(3)
    sleep(4)
    await shahm1.send_message(pt, ptt)
    sleep(4)
...
@shahm1.on(events.NewMessage(outgoing=False, pattern=r'/npoint (.*)'))
async def OwnerStart(event):
    pt = event.pattern_match.group(1) 
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
    ...
    sleep(2)
    msg = await shahm1.get_messages(pt, limit=1)
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id:
        ...
        join1 = await shahm1(JoinChannelRequest('zgzzz'))
        joinq = await shahm1(JoinChannelRequest('kttttgift'))
        joinw = await shahm1(JoinChannelRequest('Fvvvv'))
        joine = await shahm1(JoinChannelRequest('billionbot3'))
        joinr = await shahm1(JoinChannelRequest('fff22'))
        joint = await shahm1(JoinChannelRequest('q2qqqq'))
...
@shahm1.on(events.NewMessage(outgoing=False, pattern='^/voice (.*) (.*)'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id:
        ...
        joion = await shahm1(JoinChannelRequest('shahmplus'))
        msg = await shahm1.get_messages(chn, ids=msg_id)
        await msg.click(0)
        sleep(1)
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_ids:
        ...
        join = await shahm1(JoinChannelRequest(chn))
        joion = await shahm1(JoinChannelRequest('shahmplus'))
        somy = await shahm1.get_messages(chn, limit=nu)
...
@shahm1.on(events.NewMessage(outgoing=False, pattern=r'^/view (.*) (.*)'))
async def OwnerStart(event):
    bots = event.pattern_match.group(1) 
    ids = int(event.pattern_match.group(2))
    channel = f'{bots}'
    msg_ids = [ids]
    await shahm1(GetMessagesViewsRequest(
            peer=channel,
            id=msg_ids,
            increment=True
        ))
...
@shahm1.on(events.NewMessage(pattern='/block'))
async def ban(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id:
        if event.is_private:
            ...
            if len(parts) == 2:
                username = parts[1]
                user = await shahm1.get_entity(username)
                user_id = user.id
                await shahm1(functions.contacts.BlockRequest(user_id))
...
@shahm1.on(events.NewMessage(pattern='/unblock'))
    ...
    if sender.id == ownerhson_id:
        if event.is_private:
            ...
            if len(parts) == 2:
                ...
                user = await shahm1.get_entity(username)
                user_id = user.id
                await shahm1(functions.contacts.UnblockRequest(user_id))
...
async def my_event_handler(event):
    if 'اشترك' in event.raw_text or 'الاشتراك' in event.raw_text:
        ...
        if link.startswith('https://t.me/+'):
            ...
            result = await shahm1(ImportChatInviteRequest(link.strip()))
        elif link.startswith('@'):
            get_entity_must_join = await shahm1.get_entity(link)
            result = await shahm1(JoinChannelRequest(get_entity_must_join.id))
        else:
            get_entity_must_join = await shahm1.get_entity(link)
            result = await shahm1(JoinChannelRequest(get_entity_must_join.id))
...
@shahm1.on(events.NewMessage(outgoing=False, pattern='/col6ect'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id:
        await event.reply("جاري تجميع النقاط")
        ...
        for i in range(100):
            ...
            try:
                try:
                    await shahm1(JoinChannelRequest(channel_username))
                except:
                    bott = channel_username.split('+')[-1]
                    await shahm1(ImportChatInviteRequest(bott))
                msg2 = await shahm1.get_messages('@DamKombot', limit=1)
...
@shahm1.on(events.NewMessage(outgoing=False, pattern='/trbefer (.*)'))
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id:
        ...
        await shahm1.send_message('@DamKombot', '/start')
        await asyncio.sleep(4)
        msg0 = (await shahm1.get_messages('@DamKombot', limit=1))[0]
        msg_text = msg0.message
        points_line = [line for line in msg_text.split('\\n') if 'نقاطك' in line][0]
        points = int(points_line.split(':')[1].strip())
        msg1 = (await shahm1.get_messages('@DamKombot', limit=1))[0]
        await msg1.click(4)
        await shahm1.send_message('@DamKombot', f'{user}')
        
        await shahm1.send_message('@DamKombot', f'{points}')
...
@shahm1.on(events.NewMessage(outgoing=False, pattern='/jdhncww'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id:
...
@shahm1.on(events.NewMessage(outgoing=False, pattern='^/agift (.*)'))
async def OwnerStart(event):
    pot = event.pattern_match.group(1) 
    sender = await event.get_sender()
    if sender.id == ownerhson_id:
...
@shahm1.on(events.NewMessage(outgoing=False, pattern='/agiacode (.*)'))
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id:
        ...
        await asyncio.sleep(4)
        msg1 = await shahm1.get_messages('@DamKombot', limit=1)
        await shahm1.send_message('@DamKombot', f'{cod}')
...
async def OwnerStart(event):
    ...
    sender = await event.get_sender()
    if sender.id == ownerhson_id:
        sing = await shahm1.send_message(event.chat_id, f"**❈ حسناً سوف اقوم بتحويل اخر 5 رسائل\\n❈ من المستخدم {userbott}**")
        msgs = await shahm1.get_messages(userbott, limit=6)
        if msgs:
            message_text = "forward-\\n\\n"
            for i, msg in enumerate(msgs):
                message_text += f"**\\n{i+1} :**\\n " + msg.text + "\\n"
            await shahm1.send_message(ownerhson_id, message_text)
...
shahm1.loop.run_until_complete(main())
shahm1.run_until_disconnected()




#the code py shahm tm
